...
- On this page
- Supported Versions
- Collaborative planning Step 1: Request a plan Step 2: Refine the plan (optional) Step 3: Approve and execute
- Visualization
- Supported tools Google Search URL Context Code Execution MCP servers File Search
- Steerability and formatting
- Multimodal inputs Document understanding
- Handling long-running tasks Streaming
- Follow-up questions and interactions
- When to use Gemini Deep Research Agent
- Agent configuration
- Availability and pricing Estimated costs
...
input
=
"Summarize the content of https://www.wikipedia.org/."
,
...
```
### File Search
Give the agent access to your own data by using the File Search tool.
```
import
...
"https://generativelanguage.googleapis.com/v1beta/interactions/INTERACTION_ID?stream=true&last_event_id=LAST_EVENT_ID"
...
input
=
"Research the competitive landscape of cloud GPUs."
,
...
- Deep Research ( deep-research-preview-04-2026 ): For a typical query requiring moderate analysis, the agent might use ~80 search queries, ~250k input tokens (~50-70% cached), and ~60k output tokens. Estimated total: ~$1.00 – $3.00 per task
- Deep Research Max ( deep-research-max-preview-04-2026 ): For deep competitive landscape analysis or extensive due diligence, the agent might use up to ~160 search queries, ~900k input tokens (~50-70% cached), and ~80k output tokens. Estimated total: ~$3.00 – $7.00 per task
## Safety considerations
