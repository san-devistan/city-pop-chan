- On this page
- How it works Enable tool combination API returns parts Critical fields in returned parts Tool-specific data
...
# Match the ID from the function_call
...
Exit
(
"no getWeather function call in response"
)
...
Exitf
(
"SendMessage for turn 2 failed: %v"
,
...
Exit
(
"empty response from model in turn 2"
)
...
'{
  ...
  "contents": [{
    ...
    "parts": [{
      ...
      "text": "What is the northernmost city in the United States? What'
\'
...
'{
  ...
  "contents": [

    {
      ...
      "parts": [{"text": "What is the northernmost city in the United States? What'
\'
's the weather like there today?"}]
...
- Include the function_declarations , along with the built-in tools you want to use, to trigger the combination behavior. If you don't include function_declarations , tool context circulation will still act on the included built-in tools, as long as the flag is set.
### API returns parts
...
- id : A unique identifier that maps a call to its response. id is set on all function call responses , regardless of tool context circulation. You must provide the same id in the function response that the API provides in the function call. Built-in tools automatically share the id between the tool call and tool response. Found in all tool-related parts: toolCall , toolResponse , functionCall , functionResponse , executableCode , codeExecutionResult
- tool_type : Identifies the specific tool being used; the literal built-in tool or (e.g. URL_CONTEXT ) or function (e.g. getWeather ) name. Found in toolCall and toolResponse parts.
- thought_signature : The actual encrypted context embedded in each part returned by the API . Context can't be reconstructed without thought signatures; if you don't return the thought signatures for all parts in every turn, the model will error out. Found in all parts.
### Tool-specific data
