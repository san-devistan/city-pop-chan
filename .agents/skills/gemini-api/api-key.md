...
- Secure unrestricted API keys Restrict the key to Gemini API only Restrict the key for non-Gemini API use
- Blocked keys
...
API keys that are unrestricted are vulnerable to bad actors and unauthorized use. Starting June 19, 2026, to improve security, the Gemini API will discontinue support for unrestricted traffic keys.
This means that your Gemini API requests will fail if you don't take action.
To continue using the Gemini API without interruption, secure your traffic keys by adding restrictions in AI Studio .
In aistudio.google.com/api-keys , you will see a banner to notify you when API keys are unrestricted. You can see which keys are unrestricted, and service usage in the past 90 days.
...
If you want to restrict the key to Gemini API only, secure your key in AI Studio by clicking the Restrict to Gemini API button.
...
If you'd like to modify keys with existing or newly added restrictions, visit the Google Cloud Console .
## Blocked keys
Starting May 7, 2026, the Gemini API will block unrestricted API keys that have been dormant for an extended period of time. These users will see an Blocked tag for their key on aistudio.google.com/api-keys and will need to generate a new key or use an alternative restricted key to continue using the Gemini API.
## Troubleshooting API key creation
