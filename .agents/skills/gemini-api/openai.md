...
- Image understanding
- Generate an image
- Generate a video Check video status
- Audio understanding
- Structured output
- Embeddings
- Batch API
- Flex and Priority inference
- Enable Gemini features with extra_body Example using extra_body
- List models
- Retrieve a model
- Current limitations
- What's next
...
Video generation is a long-running operation that returns an operation ID you can poll for completion.
```
from
...
prompt
=
"A cinematic drone shot of a waterfall"
,
...
```
### Check video status
Video generation is asynchronous. Use GET /v1/videos/{id} to poll the status and retrieve the final video URL when complete:
```
import
...
## Flex and Priority inference
The Gemini API matches OpenAI's service_tier parameter in name and logic, enforcing limits and gracefully directing traffic for both the Flex and Priority inference tiers.
```
from
...
```
When not explicitly assigned, service_tier defaults to standard , equivalent to default for OpenAI. Learn more about inference tiers in the Optimization documentation.
...
https://generativelanguage.googleapis.com/v1beta/openai/models/gemini-3.5-flash
