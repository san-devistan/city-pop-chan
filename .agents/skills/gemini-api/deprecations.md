...
- Lyria models
- Robotics models
This page lists the known deprecation schedules for stable (GA) and preview models in the Gemini API. A " deprecation " is the announcement that we no longer provide support for a model, and that it will be " shut down " in the near future. Once a model is " shutdown ", it is completely turned off, and the endpoint is no longer available.
Deprecation announcements are made on the Release notes page, and the announced earliest shutdown dates are tracked on this page. Already-shutdown models are indicated with gray backgrounds.
## Gemini 3 models
