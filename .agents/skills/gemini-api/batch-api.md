# Batch API
- On this page
- Creating a batch job Inline requests Input file Batch embedding support Request configuration
- Monitoring job status Polling and webhooks
- Retrieving results
- Listing batch jobs
...
Tired of polling? Gemini now supports Webhooks for processing completions asynchronously. Instead of continuously calling GET / operations , subscribe to batch.succeeded directly to allow the Gemini API to push real-time notifications to your server when asynchronous or long-running operations complete.
```
from
...
error
(
`An unexpected error occurred writing file:`
,
...
"Job succeeded. Downloading results from 
$responses_file_name
..."
...
- Caching: Context caching is supported for batch requests. Reuse cached content by specifying the cached_content resource name in the configuration of individual requests within your batch. If a request in your batch results in a cache hit, you pay the standard context caching rates .
## Best practices
- Use input files for large requests: For a large number of requests, always use the file input method for better manageability and to avoid hitting request size limits for the BatchGenerateContent call itself. Note that there's a 2GB file size limit per input file.
